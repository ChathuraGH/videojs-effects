...
(function(){
  ...
  var DEFAULTS={ defaultIntensity:0.7, gridColumns:4, previewSize:120, livePreview:true, previewFps:6, defaultEffectCode:'none', hotkeys:{ toggleModal:'KeyE', toggleEffect:'KeyT', nextEffect:'BracketRight', prevEffect:'BracketLeft', intensityUp:'Equal', intensityDown:'Minus', toggleLivePreview:'KeyP' } };
  ...
  function plugin(options){
    ...
    controlsRow.appendChild(intensityLabel); controlsRow.appendChild(intensityRange); controlsRow.appendChild(livePreviewChip);
    var grid=el('div','vjs-effects-grid'); grid.style.gridTemplateColumns='repeat('+settings.gridColumns+', minmax(0, 1fr))';
    panel1.appendChild(controlsRow); panel1.appendChild(grid);
    var settingsGrid=el('div','vjs-effects-settings'); function makeSetting(l,i){ var w=el('div','vjs-effects-setting'); w.appendChild(el('label',null,{text:l})); w.appendChild(i); return w; }
    ...
    var sThumbUrl=el('input'); sThumbUrl.type='url'; sThumbUrl.value='https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&w=512&q=60';
    ...
    function startOverlayLoop(){ ensureOverlayRenderer(); overlayCanvas.style.display=effectEnabled?'block':'none'; if(!effectEnabled) return; var running=true; function loop(){ if(!running) return; overlayRenderer.draw(); requestAnimationFrame(loop);} requestAnimationFrame(loop); return function stop(){ running=false; }; }
    var stopOverlay=null; function updateOverlayState(){ if(effectEnabled){ if(!stopOverlay) stopOverlay=startOverlayLoop(); } else { if(stopOverlay){ stopOverlay(); stopOverlay=null; } overlayCanvas.style.display='none'; } }
    // Shared preview renderer (single GL context)
    var previewSharedCanvas=document.createElement('canvas'); previewSharedCanvas.width=settings.previewSize; previewSharedCanvas.height=settings.previewSize; var previewRenderer=null; var previewImage=new Image(); previewImage.crossOrigin='anonymous'; previewImage.src='https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&w=512&q=60';
    ...
    function createTile(effect){ var tile=el('div','vjs-effects-tile'); var head=el('div','vjs-effects-tile-header'); head.appendChild(el('div','vjs-effects-tile-name',{text: effect.name})); head.appendChild(el('div','vjs-effects-tile-code',{text: effect.code})); var canvas=el('canvas','vjs-effects-tile-canvas'); canvas.width=settings.previewSize; canvas.height=settings.previewSize; tile.appendChild(head); tile.appendChild(canvas); tile.addEventListener('click', function(){ if(effect.code==='none'){ effectEnabled=false; updateOverlayState(); } else { effectIndex=effect.index; ensureOverlayRenderer(); overlayRenderer.setEffect(EFFECTS[effectIndex]); overlayRenderer.setIntensity(parseFloat(intensityRange.value)); effectEnabled=true; updateOverlayState(); } }); previewRenders.push({ effect:effect, canvas:canvas }); return tile; }
    function buildGrid(){ grid.innerHTML=''; grid.style.gridTemplateColumns='repeat('+settings.gridColumns+', minmax(0, 1fr))'; previewRenders=[]; EFFECTS.forEach(function(eff){ grid.appendChild(createTile(eff)); }); restartPreviewTimer(); if(!settings.livePreview){ // Draw once when image is ready
        if (previewImage.complete) { drawThumbnailsOnce(); } else { previewImage.onload = drawThumbnailsOnce; }
      } }
    ...
    function activateTab(idx){ if(idx===0){ tab1Btn.classList.add('vjs-effects-active'); tab2Btn.classList.remove('vjs-effects-active'); panel1.classList.add('vjs-effects-active'); panel2.classList.remove('vjs-effects-active'); } else { tab2Btn.classList.add('vjs-effects-active'); tab1Btn.classList.remove('vjs-effects-active'); panel2.classList.add('vjs-effects-active'); panel1.classList.remove('vjs-effects-active'); } }
    tab1Btn.addEventListener('click',function(){ activateTab(0); }); tab2Btn.addEventListener('click',function(){ activateTab(1); });
    var Button=videojs.getComponent('Button'); class EffectsButton extends Button { constructor(player, options){ super(player, options); this.controlText('Effects'); } handleClick(){ toggleModal(); } buildCSSClass(){ return 'vjs-effects-button vjs-control vjs-button vjs-icon-placeholder'; } }
    videojs.registerComponent('EffectsButton', EffectsButton); player.ready(function(){ player.controlBar.addChild('EffectsButton', {}, player.controlBar.children().length - 1); });
